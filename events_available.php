...
<body>
    <div class="page-header">
        <h1>Events available to, <b><?php echo htmlspecialchars($_SESSION["username"]); ?></b>.</h1>
...
// If statement for whether or not the user speaks both languages
if($_SESSION["lang"]=='both'){
  $result = mysqli_query($link,"SELECT * FROM events WHERE event_parts < event_max ORDER BY event_date");
  ...
  echo "<td style=\"text-align: center;\"><a href=\"confirm_registration.php?event_id=" . $row['event_id'] . "&event_name=" . $row['event_name'] . "&user_id=" . $_SESSION["id"] ."\" class=\"btn btn-success\">Register</a></td>";
  echo "</tr>";
  }
  echo "</tbody>
  </table>";
...
$result = mysqli_query($link,"SELECT * FROM events WHERE (event_lang='$sessionlang' OR event_lang='both') AND event_parts < event_max ORDER BY event_date");
...
<th scope=\"col\">Event participants</th>
<th scope=\"col\">Event language</th>
<th scope=\"col\">Event Registration</th>
</tr>
</thead>
<tbody>";
